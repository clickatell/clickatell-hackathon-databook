...
class API {
	...
	//-------------------------------Other Methods-------------------------------------
    public function checkUpsert($key)
    {
        $this->API_key=$key;
        if($this->check_API_key() || !$this->API_key_required)
        {
            if($this->API_key_required)
            {
                $query = "SELECT " . $this->API_hit_date_column_name . " FROM " . Database::$users_table . " WHERE " . $this->API_key_column_name . " = '" . $key . "' LIMIT 1";
	...
	protected function form_query(&$get_array){
        ...
        {
            $range=array();
            if($parameter=='agegroup')
            {
                $range=explode(':',$value);
                $datet = strtotime('-'.$range[0].' years');
                $datef = strtotime('-'.$range[1].' years');
            }
        ...
        $apiresult = Database::get_all_results($API_key_query);
		...
		//if search was not specified as a parameter used use LIKE
            ...
            $query.=" AND user_id=".$apiresult[0]['id']." ";

            if(!empty($range))
            {
                $query.=' AND birthdate >= '.date("d-m-Y",$datet);
                $query.=' AND birthdate <= '.date("d-m-Y",$datef);
            }
