<?php
    ...
    if(isset($_GET) && !empty($_GET))
    {
        ...
        if(!empty($_GET['key']) && $api->checkUpsert($_GET['key']))
        {
            ...
            if(!empty($get))
            {
                if(empty($get['id']))
                {
                    $json_obj->error = "The id field is required";
                }
                else
                {
                    ...
                    $sql='delete from contact where id='.$post_array['id'].' and user_id='.$post_array['user_id'];
                    //submit the data to your table.
                    if($id=Database::execute_sql($sql))
                    {
                        $json_obj->success = "The contact was deleted from the database ";
                        //$json_obj->id = $id;
                    }else
                    {
                       $json_obj->error = "There was an error deleteing the data from the database";
                    }
                }
            }
            else
    ...
    {
        $json_obj->error = "Nothing was deleted from the database because the http request has no GET values";
    }
